...
def affichemot(mot) : #fonction qui affiche le mot et le nombre d'essai restants
    ...
    if reponse.upper()==mot.upper()and vies !=0 : #si le joueur à afficher le mot correctectement et que le nombre d'erreur est différent de 0 
        ...
        recommencer=input("Voulez-vous recommencer ? Oui ou Non :") #on propose de rejouer
        if recommencer=="Oui":                               #grâce à cette boucle on peut rejouer directement
                mot=trouvermot()
                affichemot(mot)            
...
def trouvermot() :                       #fonction chargée de choisir un mot aléatoirement
    ...
    with open('dico.txt', 'r') as f:  # Ouvre le fichier en mode lecture
        mots = f.readlines()          # Lit toutes les lignes du fichier dans une liste
        
    if not mots:  # Vérifie si la liste est vide
        return ""  # Retourne une chaîne vide si le fichier est vide
    
    mot = random.choice(mots).strip()  # Choisit un mot au hasard et enlève les espaces/retours à la ligne
    return mot  # Retourne le mot choisi 
